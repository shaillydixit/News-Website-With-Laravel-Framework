...
action="{{route('update.seo' , $seo->id)}}" >
  @csrf
 ...
 @error('meta_keyword')
  ...
  @enderror
   ...
   {{$seo->meta_description}}
    ...
    id="summernote" >
   {{$seo->google_analytics}}</textarea>
    </div>

    <div class="form-group">
    <label for="exampleInputEmail1">Google Verification</label>
    <input type="text" class="form-control" name="google_verfication" 
    value="{{$seo->google_verfication}}">
 @error('google_verfication')
  <span class="text-danger">{{$message}}</span>
  @enderror
    </div>
